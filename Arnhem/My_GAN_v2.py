...
def data_dist(job,N):
    ...
    tdata = np.column_stack((idx,data))
    sidx = np.random.choice(idx,N)
    fdata = tdata[sorted(sidx)]
    gen_data = gen_dist(fdata,N)
    return fdata[:,1], gen_data
    #plt.plot(xcoord,data[:,-1])
    
    
def gen_dist(d,N):
    #return (np.random.normal(size=N))
    return (np.linspace(min(d[:,0]),max(d[:,0]),N) + np.random.random(N) * 0.01)    
